...
export default function Home() {
  ...
  useEffect(() => {
    if (isAuthenticated && currentView === 'login') setCurrentView('dashboard')
    else if (!isAuthenticated && currentView !== 'login') setCurrentView('login')
      ...
      <AnimatePresence mode="wait">
        ...
        {currentView === 'dashboard' && (
          <motion.div key="dashboard" variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
            <Dashboard
              ...
              onLogout={() => { logout(); setCurrentView('login') }}
        ...
        {currentView === 'list' && selectedListId && (
          <motion.div key="list" variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
            <ListDetail listId={selectedListId} onBack={() => { setSelectedListId(null); setCurrentView('dashboard') }} />
          </motion.div>
        )}
        {currentView === 'stats' && (
          <motion.div key="stats" variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.3 }}>
